...
% --- Title ---
\title{\textbf{Gemmule: A High-Performance Pure-C Library for\\
...
We present \textbf{Gemmule}, a pure-C library implementing Expectation--Maximisation
...
Gemmule requires no external dependencies and achieves competitive or
...
scenarios (50 seeds, 95\,\% confidence intervals), Gemmule is faster in
every case (1.5$\times$--6.6$\times$) while matching sklearn accuracy in
7 of 8 scenarios.
Gemmule is released under GPL\,v3 at
...
We introduce \textbf{Gemmule}, a BSD-style C~library (C99) that addresses
...
in practice.
The name \emph{Gemmule} is drawn from biology: a gemmule is a discrete
subpopulation that buds off from a mixed whole --- just as this library
decomposes an observed mixture distribution into its constituent component parts.
...
Gemmule is organised as a single-library compilation unit
...
EM.
Gemmule uses the k-means++ seeding strategy~\citep{arthur2007kmeans},
...
Gemmule auto-dispatches to one of three code paths at library load time
...
For datasets with $n \geq 50{,}000$ observations, Gemmule optionally
...
Gemmule activates Squared Iterative Methods
...
For datasets that exceed available RAM, Gemmule provides a \emph{streaming
...
across chunks.
This extends Gemmule to arbitrarily large datasets at the cost of
...
\begin{table}[H]
\centering
\caption{Univariate distribution families supported by Gemmule.}
\label{tab:families}
\small
\begin{tabular}{ll}
\toprule
...
For multivariate data, Gemmule supports:
...
modelling.
Gemmule supports five criteria evaluated over a user-specified
...
Gemmule SIMD path active, OpenCL disabled for fair comparison).
...
Gemmule achieves strictly faster runtime in all eight scenarios (1.5$\times$
...
Gemmule and sklearn, indicating statistically equivalent accuracy.
...
\begin{table*}[t]
\centering
\caption{Benchmark results: Gemmule vs.\ \texttt{sklearn.mixture.GaussianMixture}.
  Speed is wall-clock speedup ratio (Gemmule/sklearn$^{-1}$; higher is better for Gemmule).
...
Gemmule is available under the GNU General Public License v3.0:
\begin{center}
  \url{https://github.com/mathornton01/gemmulem}
\end{center}
